...
PAGE_LOAD_TIMEOUT = 30   # seconds to wait for table to appear
...
def wait_for_table(driver: webdriver.Chrome, timeout: int = PAGE_LOAD_TIMEOUT):
    ...
    WebDriverWait(driver, timeout).until(
        EC.presence_of_element_located(
            (By.CSS_SELECTOR, "table tbody tr td.dtr-control")
        )
    )
...
def set_entries_per_page(driver: webdriver.Chrome, n: int = ENTRIES_PER_PAGE):
    ...
    try:
        ...
        time.sleep(2)           # let DataTable re-render
...
def go_to_next_page(driver: webdriver.Chrome) -> bool:
    ...
    """
    # DataTables renders Next as either <a> or <button> with these classes
    selectors = [
        "a.paginate_button.next",
        "button.paginate_button.next",
        "li.paginate_button.next a",
        "#DataTables_Table_0_next",        # default DataTables ID
        "[id$='_next']",                   # any element whose id ends in _next
    ]
    try:
        next_btn = None
        ...
        # Grab first row text before clicking so we can confirm the page changed
        try:
        ...
        # Scroll the button into view then click via JS to bypass any
        # overlapping elements (sticky headers, cookie banners, footers, etc.)
        driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", next_btn)
        time.sleep(0.5)
        driver.execute_script("arguments[0].click();", next_btn)

        # Wait until the first row text changes — confirms DataTable re-rendered
        def page_changed(d):
            try:
                first_row = d.find_element(
                    By.CSS_SELECTOR, "table tbody tr td.dtr-control"
                ).text.strip()
                return first_row != first_row_before and first_row.startswith("DKI")
            except Exception:
                return False
...
def append_csv(records: list[dict]) -> None:
    ...
    # Final safety net: never write rows that aren't real DKI station entries
...
if __name__ == "__main__":
    ...
    if is_first_run:
        print("[INIT] No existing CSV found — first run. Writing all records.")
        save_json(data)
        append_csv(data)
